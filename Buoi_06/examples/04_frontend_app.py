"""
Ví dụ 4: Xây dựng Dashboard Vận hành Hoàn chỉnh bằng Streamlit kết nối Backend API
Chủ đề: Hệ thống AI Phân tích & Phản hồi Đánh giá Khách hàng (Customer Feedback AI)
Giáo án Team AI ProPTIT D24 - Buổi 06

Mục tiêu học tập:
1. Kết nối giao diện người dùng Streamlit tới Backend FastAPI thông qua thư viện requests.
2. Xử lý trọn vẹn 2 phương thức giao tiếp: RESTful POST (JSON tiêu chuẩn) và GET Streaming (Chunked transfer).
3. Làm chủ kỹ thuật bắt ngoại lệ mạng (ConnectionError, Timeout) và hiển thị thông báo lỗi thân thiện.
4. Quản lý trạng thái tương tác và lịch sử phân tích với st.session_state.
"""
...
# ==============================================================================
# 2. KHỞI TẠO SESSION STATE ĐỂ LƯU TRỮ DỮ LIỆU PHIÊN
# ==============================================================================
...
with st.sidebar:
    ...
    # Nút Ping kiểm tra kết nối tới server
    if st.button("🔍 Kiểm tra kết nối API"):
        try:
            ...
            st.sidebar.error("Không tìm thấy server! Hãy chắc chắn bạn đã chạy file `03_backend_api.py`.")
...
with tab1:
    ...
    # Các nút nạp nhanh câu mẫu giúp giảng viên và học viên demo tiện lợi
    ...
    if st.button("🚀 Gửi phân tích lên Backend API", type="primary"):
        if not user_review.strip():
            ...
            with st.spinner("Đang gửi HTTP POST request tới FastAPI Backend..."):
                ...
                try:
                    # 1. Đóng gói Payload theo đúng Schema ReviewInferenceRequest của Pydantic
                    payload = {
                        "review_text": user_review.strip(),
                        "product_category": category
                    }
                    
                    # 2. Gửi HTTP POST request qua thư viện requests
                    response = requests.post(
                        f"{api_url}/predict",
                        json=payload,
                        timeout=5.0  # Timeout an toàn tránh treo ứng dụng
                    )
                    latency_ms = (time.time() - start_time) * 1000
                    
                    # 3. Kiểm tra mã trạng thái HTTP trả về
                    if response.status_code == 200:
                        ...
                        # Hiển thị khuyến nghị hành động cho CSKH
                    ...
                    st.error("🚨 **Lỗi kết nối (ConnectionError):** Không thể kết nối tới Backend API! "
                             "Hãy đảm bảo bạn đã chạy file `03_backend_api.py` bằng Uvicorn trên cổng 8000.")
                except requests.exceptions.Timeout:
                    st.error("⏳ **Lỗi quá thời gian chờ (Timeout):** Máy chủ không phản hồi trong 5 giây.")


# ==============================================================================
# TAB 2: KẾT NỐI API STREAMING (SERVER-SENT STREAMING VIA ST.WRITE_STREAM)
# ==============================================================================
with tab2:
    ...
    if st.button("✍️ Soạn thư phản hồi ngay (Streaming)", type="primary"):
        if not stream_review.strip():
            ...
            try:
                # Định nghĩa Generator phía Client để nhận từng chunk qua HTTP Stream
                def customer_reply_streamer():
                    with requests.get(
                        f"{api_url}/predict-stream",
                        params={"review_text": stream_review.strip()},
                        stream=True,  # BẮT BUỘC: Giữ kết nối TCP mở
                        timeout=10.0
                    ) as resp:
                ...
                # st.write_stream tự động đọc generator và tạo hiệu ứng gõ chữ
...
# ==============================================================================
# 5. HIỂN THỊ BẢNG LỊCH SỬ PHÂN TÍCH TRONG PHIÊN LÀM VIỆC
# ==============================================================================
st.divider()
...
if st.session_state.history:
    ...
    # Cho phép tải lịch sử về máy dạng CSV
